...
- **캐릭터 추가 (`/add`)**: 수기 입력 폼 (서버 슬러그 자동완성 + 직업 선택 + 메모) / CSV 업로드 탭. CSV는 파일 선택·붙여넣기·샘플 채우기 지원, 파싱 오류는 라인별로 표시, 파싱 결과 프리뷰 후 일괄 추가.
...
3. Framework Preset은 자동으로 **Next.js** 인식, Root Directory 기본값(레포 루트) 그대로 두면 됩니다.
4. Build Command / Output Directory 등도 기본값 사용. 환경 변수 설정 없음.
5. `_kotlin_legacy/`는 `.gitignore`/`tsconfig.json` exclude와 lint ignore에 포함돼 있어 빌드에서 제외됩니다.
